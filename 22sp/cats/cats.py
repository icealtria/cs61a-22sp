    ...
    def choose_helper(k, index):
        ...
        if select(paragraphs[index]):
            if k == 0:
                ...
                return choose_helper(k-1,index+1)
        return choose_helper(k,index+1)
    return choose_helper(k, 0)
    # END PROBLEM 1
...
def about(topic):
    ...
    def is_related(p):
        for word in topic:
            if word in remove_punctuation(lower(p)).split():
                return True
        return False
    return is_related
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    def accuracy_helper(index,count):
        ...
        try:
            if typed_words[index]==reference_words[index]:
                return accuracy_helper(index+1,count+1)
            return accuracy_helper(index+1,count)
        except IndexError:
            return scoring()
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    dict = {}
    for w in word_list:
        dict[w] = diff_function(typed_word, w, limit)
    if min(dict.values())<=limit:
...
def time_per_word(words, times_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    return {"words":words, "times": [list(map(sub,p[1:],p[:-1])) for p in times_per_player]}
    # END PROBLEM 9
