...
class TutorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        added = False
        if player.hand:
            player.hand.append(Card.copy(player.hand[0]))
            added = True
        opponent_card.attack = float('inf')
        # END Problem 4
        # You should add your implementation above this.
        if added:
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player:Player, opponent, arg=None):
        """
        ...
        best_card = None
        if player.hand:
            # best_card = max(player.hand, key=lambda x:x.attack-opponent_card.defense)
            best_card = max(filter(lambda c: type(c) is not TutorCard, player.hand),
                            key=lambda x: x.attack-opponent_card.defense)
