...
def do_define_form(expressions, env:Frame):
    ...
    if scheme_symbolp(signature):
        ...
        de_lambda = do_lambda_form(Pair(formals, body), env)
        env.bindings[name] = de_lambda
        return name
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    while bindings:
        sub_binding = bindings.first
        validate_form(sub_binding, 2, 2)
        names = Pair(sub_binding.first, names)
        vals = Pair(scheme_eval(sub_binding.rest.first, env), vals)
